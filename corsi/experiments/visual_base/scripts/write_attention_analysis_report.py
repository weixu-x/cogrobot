...
QUESTIONS = [
    ...
    "Decay/capacity: check for span cliffs in accuracy_len_6 to accuracy_len_7 and concentrated first-error positions.",
...
def main() -> None:
    ...
    for model_name, model_rows in sorted(grouped.items()):
        ...
        lines.append("")
        lines.append("| eval_mode | token_acc | full_seq_acc | entropy | peak_abs | local_w1 | repeat | wrong | order |")
        lines.append("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
        for row in model_rows:
            lines.append(
                "| {eval_mode} | {token_accuracy} | {full_sequence_accuracy} | {mean_attention_entropy} | "
                "{mean_peak_displacement_abs} | {local_mass_w1} | {repeat_error_rate} | "
                "{wrong_block_rate} | {order_error_rate} |".format(
